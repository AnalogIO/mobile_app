...
class BuyTicketsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return RepositoryProvider(
      create: (context) => TicketCatalogRepository(
        remoteDataProvider: TicketCatalogRemoteDataProvider(
          executor: context.read(),
        ),
        drinksLocalDataProvider: DrinksLocalDataProvider(),
      ),
      child: BlocProvider(
        create: (context) {
        ...
        child: BlocBuilder<BuyTicketsCubit, BuyTicketsState>(
          builder: (context, state) {
            return switch (state) {
              BuyTicketsInitial() ||
              BuyTicketsLoading() => const Screen.withBody(
                name: 'Buy tickets',
                body: Center(
